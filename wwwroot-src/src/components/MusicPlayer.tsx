...
// Import backend functions conditionally
let GetMusicEnabled: () => Promise<boolean>;
let SetMusicEnabled: (enabled: boolean) => Promise<boolean>;

try {
  const backend = await import('../../wailsjs/go/app/App');
  GetMusicEnabled = backend.GetMusicEnabled;
  SetMusicEnabled = backend.SetMusicEnabled;
} catch {
